...
class ImageINRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(model_path):
            ...
            if self.on_the_fly:
                state_dict = self.trainer.fit_inr(
                    self.dataset, index, self.model_cls, 42
                )
                model = self.model_cls(
                    seed=42, INR_model_config=self.trainer.INR_model_config
                ).to(self.device)
                ...
                raise FileNotFoundError(
                    f"Model for index {index} not found and on_the_fly is set to False."
                )
...
def main():
    ...
    img_inr_dataset = ImageINRDataset(
        "MNIST",
        sMLP,
        inr_trainer,
