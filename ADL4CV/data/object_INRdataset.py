...
class ObjectINRDataset(Dataset):
    def __init__(self, model_cls, trainer, model_save_dir, on_the_fly=False, path=None, device='cpu'):
    ...
    def __getitem__(self, index):
        obj = np.genfromtxt(f"ADL4CV/data/gt_data/shrec_16_processed_collapsed/T{index}.xyz")
        obj = torch.tensor(obj)
        obj = obj.to(self.device)  # Move the image to the specified device
