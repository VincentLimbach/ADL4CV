...
from ADL4CV.architectures import sMLP, HyperNetwork2D, SharpNet
...
class HyperNetworkTrainer:
    ...
    def process_batch(self, batch, criterion, final=False, batch_results=False):
        (
            img_1_batch,
            label_1_batch,
            flat_weights_1_batch,
            img_2_batch,
            label_2_batch,
            flat_weights_2_batch,
        ) = [b for b in batch]
        ...
        predicted_weights = self.hypernetwork(
            concatenated_weights, offsets, label_1_batch, label_2_batch
        )
    ...
    def train_one_epoch(self, dataloader, criterion, optimizer):
        ...
        for batch in dataloader:
            ...
            batch_loss, _ = self.process_batch(batch, criterion)
    ...
    def validate(self, dataloader, criterion, final, batch_results=False):
    ...
    def train_hypernetwork(
        ...
        weak_val_loader = DataLoader(
            weak_val_dataset, batch_size=batch_size, shuffle=False
        )
        strong_val_loader = DataLoader(
            strong_val_dataset, batch_size=batch_size, shuffle=False
        )
        ...
        optimizer = optim.Adam(
            list(self.hypernetwork.parameters()) + list(self.sharpnet.parameters()),
            lr=0.00005,
        )
        ...
        for epoch in range(epochs):
            ...
            weak_val_loss, weak_val_results = self.validate(
                weak_val_loader, criterion, final=final
            )
            strong_val_loss, strong_val_results = self.validate(
                strong_val_loader, criterion, final=final
            )
            self.writer.add_scalars(
                datetime.now().strftime("%Y%m%d-%H%M%S"),
                {
                    "Train": train_loss.item(),
                    "Weak_Val": weak_val_loss.item(),
                    "Strong_Val": strong_val_loss.item(),
                },
                epoch,
            )
            ...
            if epoch == 0 or epoch == 1 or epoch % 10 == 0:
                ...
                print(
                    f"Epoch [{epoch + 1}/{epochs}], Train: {train_loss.item()}, Weak_Val: {weak_val_loss.item()}, Strong_Val: {strong_val_loss.item()}"
                )
...
def main():
    ...
    weak_val_results, strong_val_results = hypernetwork_trainer.train_hypernetwork(
        "MNIST",
        train_pairs=train_pairs,
        weak_val_pairs=weak_val_pairs,
        strong_val_pairs=strong_val_pairs,
        on_the_fly=True,
    )
