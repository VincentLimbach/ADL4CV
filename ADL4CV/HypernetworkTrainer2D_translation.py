...
class HyperNetworkTrainer:
    def __init__(self, hypernetwork_translation, base_model_cls, inr_trainer, save_path, load=False, override=False):
        with open("config.json") as file:
        ...
        if load:
            if os.path.exists(self.save_path):
                self.hypernetwork_translation.load_state_dict(torch.load(self.save_path, map_location=torch.device('cpu')))
    ...
    def process_batch(self, batch, criterion, debug=False, final=False):
        ...
        offsets = torch.randint(1, 16, (img_1_batch.size(0), 2), device=device) * 2
        ...
        translation_loss = translation_loss_1

        img_shifted = generate_shifted_image(img_1_batch, offsets[:, 0], offsets[:, 1], device)
        height, width = img_shifted.shape[1], img_shifted.shape[2]
        coords = torch.cartesian_prod(torch.arange(height, device=device), torch.arange(width, device=device)).float()
        
        predictions = new_model(coords, weights=weights_1, biases=biases_1)
    ...
    def train_hypernetwork(self, dataset_name, train_pairs, weak_val_pairs, strong_val_pairs, on_the_fly=False, debug=False, batch_size=512, path_dic=None, path_model=None):
        ...
        if not self.load:
            optimizer = optim.Adam(self.hypernetwork_translation.parameters(), lr=0.0005)
            ...
            torch.save(self.hypernetwork_translation.state_dict(), self.save_path)
...
def main():
    ...
    hypernetwork_trainer = HyperNetworkTrainer(hypernetwork_translation, sMLP, inr_trainer, save_path='ADL4CV/models/2D/hypernetwork_translation.pth', override=True)
    ...
    train_pairs, weak_val_pairs = generate_pairs(128, 16, 8, seed=42)
    strong_val_pairs, _ = generate_pairs(16, 0, 1, seed=42)
    strong_val_pairs= [(i+8192, j+8192) for i, j in strong_val_pairs]
    ...
    weak_val_results, strong_val_results = hypernetwork_trainer.train_hypernetwork("MNIST", train_pairs=train_pairs, weak_val_pairs=weak_val_pairs, strong_val_pairs=strong_val_pairs, on_the_fly=True, debug=True, batch_size=32)
