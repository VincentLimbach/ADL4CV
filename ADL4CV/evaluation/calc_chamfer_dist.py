...
with open("config.json") as file:
    model = sMLP(seed=42, INR_model_config=json.load(file)["INR_model_config_3D"], device=device)
...
for pair in train_pairs:
    idx_1, idx_2 = pair
    pointcloud_1 = np.genfromtxt(f"ADL4CV/data/gt_data/shrec_16_processed_collapsed/T{idx_1}.xyz")
    pointcloud_2 = np.genfromtxt(f"ADL4CV/data/gt_data/shrec_16_processed_collapsed/T{idx_2}.xyz")
    v_1 = pointcloud_1[:, :3]
    v_2 = pointcloud_2[:, :3]

    v_1 = normalize(v_1) + np.array([[0, -.5, 0]])
    v_2 = normalize(v_2) + np.array([[0, .5, 0]])
