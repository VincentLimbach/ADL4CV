...
def train_INRpair2D(dataset, pair_index, offsets, model_cls, seed, save=False, save_name=None):
    img_1, _ = dataset[pair_index[0]]
    img_2, _ = dataset[pair_index[1]]

    img_concat = generate_merged_image(img_1, img_2, offsets[:, 0], offsets[:, 1], offsets[:, 2], offsets[:, 3], device)
    ...
    optimizer = optim.Adam(model.parameters(), lr=INR_trainer_config["lr"], weight_decay=INR_trainer_config.get("weight_decay", 0))
...
dataset = datasets.MNIST(root='ADL4CV/data/gt_data/', train=True, download=True, transform=transforms.ToTensor())
