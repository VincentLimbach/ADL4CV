...
def generate_merged_image(
    img_1_batch, img_2_batch, x_1_offsets, y_1_offsets, x_2_offsets, y_2_offsets, device
):
    ...
    merged_imgs = torch.zeros(
        (batch_size, max_height, max_width), dtype=torch.float32, device=device
    )
    ...
    merged_imgs[batch_indices, y1_indices[:, :, None], x1_indices[:, None, :]] = (
        img_1_batch
    )
...
def generate_shifted_image(img_batch, y_1_offsets, x_1_offsets, device):
    ...
    max_width = img_width + x_1_offsets.max().item()

    shifted_imgs = torch.zeros(
        (batch_size, max_height, max_width), dtype=torch.float32, device=device
    )
    ...
    shifted_imgs[batch_indices, y1_indices[:, :, None], x1_indices[:, None, :]] = (
        img_batch
    )
...
def generate_merged_object(
    ...
    n_norm_1[n_norm_1 == 0] = 1.0
    n_1_batch = n_1_batch / n_norm_1
    offsets_1 = (
        torch.tensor([x_1_offsets, y_1_offsets, z_1_offsets], device=device)
        .unsqueeze(0)
        .unsqueeze(0)
    )
    ...
    n_norm_2[n_norm_2 == 0] = 1.0
    n_2_batch = n_2_batch / n_norm_2
    offsets_2 = (
        torch.tensor([x_2_offsets, y_2_offsets, z_2_offsets], device=device)
        .unsqueeze(0)
        .unsqueeze(0)
    )
    ...
    kd_trees = [KDTree(np.array(v_batch[i])) for i in range(batch_size)]

    points = np.repeat(np.array(v_batch), repeats=16, axis=1)
    points[:, ::2] += np.random.laplace(
        scale=1e-1, size=(batch_size, points.shape[1] // 2, points.shape[-1])
    )
    points[:, 1::2] += np.random.laplace(
        scale=1e-3, size=(batch_size, points.shape[1] // 2, points.shape[-1])
    )
    ...
    for i in range(batch_size):
        ...
        sdf = np.sum(
            (points[i] - np.array(v_batch)[i, idx][:, 0]) * avg_normal, axis=-1
        )
        sdf = sdf[..., None]
        sdfs.append(sdf)
