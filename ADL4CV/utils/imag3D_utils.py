...
def apply_sdf(
    model, len_dims_x, len_dims_y, len_dims_z, coordinates, weights=None, biases=None
):
    model.eval()
...
def generate_coordinates(step, scale_x, scale_y, scale_z):
    coordinates = [
        (-scale_x + x * step, -scale_y + y * step, -scale_z + z * step)
...
def generate_merged_gt_3D(obj_idx_1, obj_idx_2, offsets, output_file_path):
    ...
    with open(
        f"ADL4CV/data/gt_data/shrec_16_original/armadillo/train/T{obj_idx_1}.obj", "r"
    ) as file:
        for line in file:
    ...
    with open(
        f"ADL4CV/data/gt_data/shrec_16_original/armadillo/train/T{obj_idx_2}.obj", "r"
    ) as file:
        for line in file:
            if line.startswith("v "):
                parts = line.split()
    ...
    vertices[n_vertex_1:] = (
        normalize(vertices[n_vertex_1:]) + np.array([[x_2, y_2, z_2]])
    ).tolist()
