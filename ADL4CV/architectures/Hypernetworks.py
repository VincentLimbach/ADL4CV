...
class GroupLinearLayer(nn.Module):
    def __init__(self, in_features, out_features, groups=8):
        ...
        self.weight = nn.Parameter(
            torch.Tensor(groups, self.group_out_features, self.group_in_features)
        )
    ...
    def forward(self, x):
        ...
        ), f"Expected input features {self.in_features}, but got {num_features}"

        x = x.view(batch_size, self.groups, self.group_in_features)

        out = torch.bmm(x, self.weight.permute(0, 2, 1)) + self.bias.unsqueeze(
            0
        ).unsqueeze(0)
...
class HyperNetworkGrouped(nn.Module):
    def __init__(self):
        super(HyperNetworkGrouped, self).__init__()
        ...
        self.fc4 = GroupLinearLayer(
            self.inital_size + 768, hypernetwork_config["INRs_size"]
        )
...
class HyperNetwork2D(nn.Module):
    def __init__(self, dropout_rate=0.5):
        super(HyperNetwork2D, self).__init__()
        with open("config.json") as file:
        ...
        self.shared_fc1 = nn.Linear(
            self.hypernetwork_config["INRs_size"] + self.d_model * 2, 256
        )
        ...
        self.dim = (
            self.hypernetwork_config["INRs_size"] * 2
            + self.d_model * 4
            + self.shared_output_dim * 2
            + self.class_embedding_dim * 2
        )
    ...
    def forward(self, x, offsets, label_1_batch, label_2_batch):
        ...
        xs_left = torch.cat(
            (xs_left, encoded_offset[:, : encoded_offset.size(1) // 2]), dim=1
        )
        xs_right = torch.cat(
            (xs_right, encoded_offset[:, encoded_offset.size(1) // 2 :]), dim=1
        )
...
class SharpNet(nn.Module):
    ...
    def forward(self, x_org):
        ...
        return torch.clip(
            x_org * self.W_org + x_sharp.squeeze(1) * (1 - self.W_org), min=0, max=1
        )
